"""
dropout_sensitivity_sequential.py — Run one at a time to avoid OOM
...
def run_one_dropout(dropout_rate):
    ...
    for seed in SEEDS:
        ...
        if torch.cuda.is_available():
            ...
            print(f"  [GPU] cleared: {torch.cuda.max_memory_allocated() / 1e6:.1f} MB")
...
if __name__ == "__main__":
    print("="*70)
    print("  Dropout Rate Sensitivity Ablation (Sequential)")
    ...
    for dropout in DROPOUTS:
        results_d = run_one_dropout(dropout)
    ...
    for dropout in DROPOUTS:
        ...
        da_mean = subset["dir_acc"].mean().round(1)
        da_std = subset["dir_acc"].std().round(2)
