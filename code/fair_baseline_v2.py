"""
fair_baseline_v2.py — 修复版公平baseline对比
修复：HMM-MoE在每个seed下独立运行，不受前面neural模型的GPU状态污染
策略：HMM-MoE用debug_seed42.py同款代码确保复现性
"""
...
class CNNBaseline(nn.Module):
    def __init__(self, inp, hid=64, dropout=0.2):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv1d(hid, hid, 3, padding=1), nn.ReLU(), nn.AdaptiveAvgPool1d(1))
        self.fc = nn.Linear(hid, 1)
    def forward(self, x):
...
def train_neural(model, train_ld, val_ld, epochs=100, patience=15, lr=1e-3, device='cpu', loss_fn=None):
    model.to(device)
    ...
    for ep in range(epochs):
        ...
        for bx, by in train_ld:
            loss = loss_fn(model(bx.to(device)), by.to(device))
...
def eval_neural(model, X_test, sc_y, base_p, true_p, device='cpu'):
    model.eval()
    with torch.no_grad():
        pred_sc = model(to_t(X_test).to(device)).cpu().numpy().flatten()
    pred_r = sc_y.inverse_transform(pred_sc.reshape(-1,1)).flatten()
    n = min(len(pred_r), len(base_p), len(true_p))
    pred_r, bp, tp = pred_r[:n], base_p[:n], true_p[:n]
    pred_p = bp * np.exp(pred_r)
    r_true = np.log(tp / bp)
    return {'da': np.mean((pred_r * r_true) > 0) * 100,
...
def main():
    ...
    for seed in SEEDS:
        ...
        # ========== HMM-MoE FIRST (clean GPU state) ==========
        print(f"\n  [HMM-MoE] seed={seed}...", end=" ", flush=True)
        t0 = time.time()
        set_seed(seed)
        ...
        _, _, _, _, _, metrics = evaluate_moe_model(
            hmm_model, test_loader, sc_y, base_p, true_p, device=device)

        results.append({'model': 'HMM-MoE', 'seed': seed, 'loss': 'dir_aware',
        ...
        loss_fn = DirectionAwareLoss(alpha=1.0)
        train_ld = DataLoader(TensorDataset(to_t(X_train), to_t(Y_train).unsqueeze(1)), batch_size=32, shuffle=True)
        val_ld = DataLoader(TensorDataset(to_t(X_val), to_t(Y_val).unsqueeze(1)), batch_size=32)

        # ========== Neural baselines (with direction-aware loss) ==========
        for name, builder in NEURAL_MODELS.items():
            print(f"  [{name}] seed={seed}...", end=" ", flush=True)
            t0 = time.time()
            set_seed(seed)
            model = builder(n_features)
            model = train_neural(model, train_ld, val_ld, 100, 15, 1e-3, device, loss_fn)
            m = eval_neural(model, X_test, sc_y, base_p, true_p, device)
            results.append({'model': name, 'seed': seed, 'loss': 'dir_aware',
                           'da': m['da'], 'mae': m['mae'], 'mape': m['mape'],
                           'runtime': time.time()-t0})
            print(f"DA={m['da']:.1f}%  MAE={m['mae']:.4f}")
            del model
        ...
        SKLEARN = {
            ...
            'GradBoost': GradientBoostingRegressor(n_estimators=200, max_depth=5, learning_rate=0.05, random_state=seed),
        ...
        for name, mdl in SKLEARN.items():
            ...
            mae = mean_absolute_error(tp, pred_p)
            results.append({'model': name, 'seed': seed, 'loss': 'MSE_native',
                           'da': da, 'mae': mae, 'mape': np.mean(np.abs((tp-pred_p)/tp))*100,
                           'runtime': time.time()-t0})
    ...
    df.to_csv(os.path.join(output_dir, "fair_baseline_v2.csv"), index=False)

    print(f"\n{'='*70}")
    print(f"  FAIR BASELINE V2 — 3-SEED MEAN ± STD")
    print(f"{'='*70}")
    print(f"  {'Model':<20} {'Loss':<12} {'DA (%)':<18} {'MAE':<15}")
    print(f"  {'-'*65}")
    for name in df['model'].unique():
        sub = df[df['model'] == name]
        lt = sub['loss'].iloc[0]
        dm, ds = sub['da'].mean(), sub['da'].std()
        mm, ms = sub['mae'].mean(), sub['mae'].std()
        print(f"  {name:<20} {lt:<12} {dm:.1f} ± {ds:.1f}      {mm:.2f} ± {ms:.2f}")
    print(f"\nSaved to {output_dir}/fair_baseline_v2.csv")
