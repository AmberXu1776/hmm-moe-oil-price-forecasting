"""
run_one_seed_cpu.py — Run ONE SEED on CPU, no OOM ever
Usage: python run_one_seed_cpu.py <dropout> <seed_index>
"""
...
N_EXPERTS = 3
N_REGIMES = 3
D_MODEL = 32  # Back to original model size, CPU doesn't care

def main():
    if len(sys.argv) != 3:
        print("Usage: python run_one_seed_cpu.py <dropout> <seed_index 0-2>")
        print(f"  SEEDS = {SEEDS}")
        sys.exit(1)
    ...
    print("="*70)
    print(f"  Run ONE SEED on CPU: dropout={dropout}, seed={seed} (idx={seed_idx})")
    print("="*70)
    
    set_seed(seed)
    device = "cpu"  # Force CPU, no GPU memory issues
